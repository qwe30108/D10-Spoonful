...
function SearchResults() {
  ...
  const getSearchDishes = async () => {
    try {
      ...
      const filterDishes = response.data.filter((dish) => {
        const hasTagsdTag = tagsArray.length > 0 ? dish.tags.some((tag) => tagsArray.includes(tag)) : false;
        const restaurantAddress = dish.restaurant?.address || '';
        const isInAllowedCity = searchCity
          ? restaurantAddress.includes(searchCity) || searchCity.includes(restaurantAddress)
          : true;
  ...
  const ingredientsList = ['蔥', '薑', '蒜', '香菜', '洋蔥', '辣', '番茄', '生菜', '牛肉'];
  const citiesList = ['台北市', '新北市', '桃園市', '新竹縣', '苗栗縣', '台中市', '彰化縣'];
    ...
    <>
        ...
        <div className='search-box px-20 py-16 mx-auto'>
          <div className='search-box-left row'>
              ...
              <div className='dropdown' id='myDropdown'>
                ...
                >
                  <div className='selected-items d-flex align-items-end flex-wrap gap-1'>
                    {tags.length > 0 ? (
                      tags.map((item) => (
                        <span key={item} className='badge my-selected-badge d-flex align-items-center'>
                          {item}
                          <span
                ...
                <ul className={`dropdown-menu ${isTagsOpen ? 'show' : ''}`} aria-labelledby='multiSelectDropdown'>
                  {ingredientsList.map((ingredient, index) => (
                    <li key={index}>
                      <label className='dropdown-item d-block' style={{ cursor: 'pointer' }}>
                        <input
                          type='checkbox'
                          className='me-2'
                          value={ingredient}
            ...
            <div className='search-button col-md-3  align-self-end'>
              <Link to={targetUrl} className=' rounded-pill btn btn-secondary-700 px-24 py-12 border border-0'>
                <svg
                  xmlns='http://www.w3.org/2000/svg'
                  width='16'
                  height='16'
      ...
      <div className='container mx-auto px-8 px-lg-0 result-container'>
        ...
        <div className='d-flex justify-content-start align-items-center mb-5'>
          ...
          <Link className='btn btn-report rounded-1 d-flex align-items-center fs-5 py-1' to='/return'>
              ...
              <div className='card p-4 p-lg-6 mb-5 mb-lg-9 rounded-5 result-card-container'>
                ...
                <div className='row g-0'>
                  ...
                  <div className='col-lg-4 d-flex align-self-stretch mb-5 mb-lg-0'>
                    <img src={dish.images[0]} className='img-fluid rounded-3 img-dish' alt='...' />
                  </div>
                  ...
                  <div className='col-lg-8'>
                    <div className='card-body d-flex flex-column justify-content-between px-0 ms-0 ms-lg-9'>
                      {/* <!-- address & description --> */}
                      <div>
                        <h3 className='card-title text-primary-950 mb-3'>{dish.title}</h3>
                      ...
                      <div className='d-flex align-items-center flex-wrap flex-lg-nowrap'>
                        ...
                        <div className='ms-lg-auto mx-auto mx-lg-0 flex-grow-1 flex-lg-grow-0'>
                          ...
                          >
                            ...
                            <i data-feather='chevron-right' width='16px' height='16px'></i>
                          </Link>
                        </div>
                      </div>
                    </div>
