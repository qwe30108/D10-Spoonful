...
const HeroArticle = ({ newsList }) => {
  ...
  useEffect(() => {
    if (bgRef.current && currentNews?.images?.[0]) {
      bgRef.current.style.setProperty('--dynamic-bg', `url(${currentNews.images[0]})`);
    }
  }, [currentNews]);
            ...
            <div className='col-12 col-sm-4 py-9 py-lg-5 d-flex flex-column'>
              <div className='card-body p-0 d-flex flex-column'>
                <div className='d-flex align-items-center mb-3 small-title'>
                  <h6 className='card-text text-secondary-800 my-0'>{currentNews.category}</h6>
                ...
                <p className='card-title text-primary-950 mb-3 fs-32 fs-lg-48 fw-medium'>{currentNews.title}</p>
                <p className='card-text  mb-4 text-gray-800'>{currentNews.summary}</p>
                <div className='card-text mb-9 mb-lg-0'>
                  <NewsBadge tags={currentNews.tags} textColor='text-gray-900' bgColor='bg-white' />
                </div>
                ...
                <div className='d-flex align-items-center justify-content-between mt-auto'>
                  ...
                  <div className='news-controls'>
                    <button onClick={handlePrev} className='btn-news-ctrl shadow-sm'>
                      <span className='fs-5 fw-bold'>&lt;</span>
                    </button>
                    <button onClick={handleNext} className='btn-news-ctrl shadow-sm'>
                      <span className='fs-5 fw-bold'>&gt;</span>
                    </button>
                  </div>
                </div>
  ...
  return (
    <div className='card news-card'>
      ...
      <div className='card-body pt-0'>
        ...
        <p className='card-text text-gray-800 summary-text mb-4'>{data.summary}</p>
        <div className='card-text'>
          <NewsBadge tags={data.tags} textColor='text-primary-950' bgColor='bg-primary-50' />
        </div>
      </div>
    </div>
  );
    ...
    <section className='container py-10 py-lg-120'>
      ...
      <div className='d-flex flex-wrap align-items-center mb-8 row-gap-3'>
        <ul className='nav nav-tabs me-auto flex-grow-1'>
          {tabs.map((tab, index) => (
            <li className={`nav-item flex-fill flex-lg-grow-0 ${index < tabs.length - 1 ? 'pe-1' : ''}`} key={tab}>
              <a
...
const News = () => {
  ...
  if (newsList.length === 0) return <div className='text-center py-5'>消息載入中...</div>;
